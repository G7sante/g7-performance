...
const FORM_STRUCTURE = [
{
  ...
  questions: [
    { id: 'q1', type: 'ai_textarea', label: "Quel symptôme ou blocage limite le plus votre performance ou votre bien-être actuellement ?", aiPrompt: "Tu es un expert clinique. Reformule la plainte suivante en un terme médical ou fonctionnel précis. Sois concis." },
...
return (
  <div className="w-full bg-slate-200 h-1">
    <div className="bg-teal-600 h-full transition-all" style={{ width: `${percentage}%` }} />
  </div>
);
};
...
const handleAI = async () => { 
  setLoading(true); 
  onChange(await callGemini(`${question.aiPrompt} "${value}"`)); 
  setLoading(false); 
};

switch (question.type) {
  case 'text': 
    return <input type="text" className="w-full p-4 border rounded-xl" placeholder="Réponse..." value={value || ''} onChange={e => onChange(e.target.value)} />;
  case 'textarea': 
    return <textarea className="w-full p-4 border rounded-xl h-24" placeholder="Détails..." value={value || ''} onChange={e => onChange(e.target.value)} />;
  case 'ai_textarea': 
    return (
      <div className="relative">
        <textarea className="w-full p-4 border rounded-xl h-32 pr-12" placeholder="Décrivez..." value={value || ''} onChange={e => onChange(e.target.value)} />
        <button onClick={handleAI} disabled={loading||!value} className="absolute top-2 right-2 p-2 bg-purple-500 text-white rounded-lg disabled:opacity-50">
          {loading?<RefreshCw className="animate-spin"/>:<Sparkles/>}
        </button>
      </div>
    );
  case 'number': 
    return <input type="number" className="w-full p-4 border rounded-xl text-center text-2xl font-bold" placeholder="0" value={value || ''} onChange={e => onChange(e.target.value)} />;
  case 'slider': 
    return (
      <div className="w-full px-2 py-4">
        ...
        <input type="range" min={question.min} max={question.max} value={value || 0} onChange={e => onChange(e.target.value)} className="w-full h-3 bg-gray-200 rounded-lg cursor-pointer accent-teal-600" />
        <div className="text-center mt-2 font-bold text-xl">{value || 0}/10</div>
      </div>
    );
  case 'radio': 
    return (
      <div className="space-y-2">
        {question.options.map(opt => (
          <button key={opt} onClick={() => onChange(opt)} className={`w-full text-left p-4 rounded-xl border flex justify-between ${value === opt ? 'bg-teal-50 border-teal-500 font-bold' : 'bg-white'}`}>
            {opt}
            {value === opt && <Check className="text-teal-600"/>}
          </button>
        ))}
      </div>
    );
  case 'checkbox': {
    ...
    return (
      <div className="space-y-2">
        {question.options.map(opt => (
          <button key={opt} onClick={() => toggle(opt)} className={`w-full text-left p-4 rounded-xl border flex justify-between ${sel.includes(opt) ? 'bg-blue-50 border-blue-500 font-bold' : 'bg-white'}`}>
            {opt}
            {sel.includes(opt) && <Check className="text-blue-600"/>}
          </button>
        ))}
      </div>
    );
  }
  case 'conditional_pqrst': {
    const choice = value?.choice || "";
    const pqrst = value?.pqrst || {};
    return (
      <div className="space-y-3">
        <div className="flex gap-3">
          {question.options.map(opt => (
            <button key={opt} onClick={() => onChange({ ...value, choice: opt })} className={`flex-1 p-3 rounded-xl border font-bold ${choice === opt ? 'bg-teal-600 text-white' : 'bg-white'}`}>
              {opt}
            </button>
          ))}
        </div>
        {choice === question.trigger && <PQRSTFields value={pqrst} onChange={d => onChange({ ...value, pqrst: d })} />}
      </div>
    );
  }
  default: 
    ...
    <div className="w-full max-w-2xl bg-white rounded-3xl shadow-xl overflow-hidden min-h-[60vh] flex flex-col">
      {step === 0 && (
        <div className="p-8 flex flex-col items-center justify-center flex-1 text-center animate-in fade-in">
          {BRAND_CONFIG.logoUrl && <img src={BRAND_CONFIG.logoUrl} alt="Logo" className="h-20 mb-6 object-contain" />}
          ...
          <div className="w-full space-y-4 text-left">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <select className="p-3 border rounded" value={anamnese.sex} onChange={e => setAnamnese({...anamnese, sex: e.target.value})}>
      ...
      {step > 0 && step <= FORM_STRUCTURE.length && (
        <div className="flex-1 flex flex-col">
          ...
          <div className="p-4 border-t flex justify-between">
            <button onClick={() => setStep(s => Math.max(0, s-1))} className="px-6 py-3 font-bold text-slate-400">Retour</button>
          ...
          <div className="mt-8 w-full border-t pt-4">
            ...
            {showClinician && (
              <div className="mt-4 bg-slate-800 p-4 rounded-xl text-left shadow-2xl">
                ...
                {!aiAnalysis ? (
                  <button onClick={triggerAI} disabled={isAiLoading} className="w-full p-2 bg-indigo-600 text-white rounded text-sm mb-2 disabled:opacity-50">{isAiLoading ? "Analyse..." : "Générer Hypothèses IA"}</button>
                ) : (
